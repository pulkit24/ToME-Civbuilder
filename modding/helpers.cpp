        ...
        if (effect.Name.find("Elite costs -20%") != string::npos) {
            ...
            for (int i = 0; i < df->Techs.size(); i++) {
                if (df->Techs[i].ResearchLocation == 82) {
                    if (df->Techs[i].Name.find("Elite") != string::npos && df->Techs[i].Civ != -1) {
                        effect.EffectCommands.push_back(createEC(8, i, 17, -1, 0.8));
                    }
                }
            }
        } else if (effect.Name == "C-Bonus, -15% age up cost") {
            effect.EffectCommands.clear();
            effect.EffectCommands.push_back(createEC(8, 101, 17, -1, 0.85));
            effect.EffectCommands.push_back(createEC(8, 102, 17, -1, 0.85));
            effect.EffectCommands.push_back(createEC(8, 103, 17, -1, 0.85));
        } else if (effect.Name == "C-Bonus, Imperial cost -33%") {
            effect.EffectCommands.clear();
            effect.EffectCommands.push_back(createEC(8, 103, 17, -1, 0.66667));
            ...
            for (int i = 0; i < df->Techs.size(); i++) {
                if ((df->Techs[i].ResearchLocation == 45) || (df->Techs[i].ResearchLocation == 209)) {
                    effect.EffectCommands.push_back(createEC(8, i, 17, -1, 0.66667));
                }
            }
            ...
            for (int i = 0; i < df->Techs.size(); i++) {
                if ((df->Techs[i].ResearchLocation == 49) || (df->Techs[i].ResearchLocation == 103)) {
                    effect.EffectCommands.push_back(createEC(8, i, 13, -1, 0.5));
                }
            }
            ...
            for (int i = 0; i < df->Techs.size(); i++) {
                if (df->Techs[i].ResearchLocation == 101) {
                    effect.EffectCommands.push_back(createEC(8, i, 17, -1, 0.5));
                }
            }
            ...
            for (int i = 0; i < df->Techs.size(); i++) {
                if ((df->Techs[i].ResearchLocation == 562) || (df->Techs[i].ResearchLocation == 68) || (df->Techs[i].ResearchLocation == 584) ||
                    ...
                    effect.EffectCommands.push_back(createEC(8, i, 13, -1, 0.66667));
                }
            }
        } else if (effect.Name == "C-Bonus, Feudal cost -25%") {
            effect.EffectCommands.clear();
            effect.EffectCommands.push_back(createEC(8, 101, 17, -1, 0.75));
        } else if (effect.Name == "C-Bonus, Castle Age cost -25%") {
            effect.EffectCommands.clear();
            effect.EffectCommands.push_back(createEC(8, 102, 17, -1, 0.75));
...
void copyArchitecture(DatFile *df, int copyFrom, int copyTo) {
    for (int i = 0; i < df->Civs[copyFrom].Units.size(); i++) {
        // Copy buildings, but make sure to not copy castles
...
void multiplyEffects(DatFile *df, int effectID, int multiplier) {
    if (multiplier == 1) {
        return;
    }
...
void multiplyAllEffects(DatFile *df, vector<vector<int>> multipliedTechs) {
...
int allocateTech(DatFile *df, int techID, int civID /*, ofstream &logfile*/) {
...
vector<vector<int>> unitSets = {
    ...
    /*75 - War Chariots*/ {65, 200, 0, 2, 2150, 2151, 1962, 1980},
    /*76 - Jian Swordsmen*/ {50, 150, 0, 2, 1974, 1976},
    /*77 - Liu Bei*/ {200, 1500, 0, 3, 1966},
    /*78 - Sun Jian*/ {200, 1500, 0, 3, 1978},
    /*79 - Cao Cao*/ {200, 1500, 0, 3, 1954},
    /*80 - Xianbei Raiders*/ {45, 135, 0, 2, 1952},
    /*81 - Grenadiers*/ {75, 135, 0, 2, 1911},
    /*82 - Mounted Trebuchets*/ {225, 475, 0, 2, 1923},
    /*83 - Lou Chuan*/ {200, 700, 0, 2, 1948},
    /*84 - Hei Guang Cavalry*/ {65, 180, 0, 2, 1944, 1946},
    /*85 - Traction Trebuchets*/ {200, 600, 0, 2, 1942},
    /*86 - Fire Lancers*/ {45, 160, 0, 2, 1901, 1903},
    /*87 - Rocket Carts*/ {175, 400, 0, 2, 1904, 1907},
    /*88 - Siege Elephants*/ {150, 260, 0, 2, 1744, 1746},
    /*89 - Pastures*/ {50, 200, 0, 3, 1889, 1893, 1897}};

vector<int> uniqueUnits = {8,    530,  281,  531,  41,   555,  25,   554,  291,  560,  73,   559,  40,   553,  282,  556,  239,  558,  46,   557,  692,
